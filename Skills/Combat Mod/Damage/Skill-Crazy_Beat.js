...
AttackEvaluator.HitCritical.calculateDamage = function(virtualActive, virtualPassive, entry) {
	...
	var boost = Math.floor(damage*0.2) //Modify the 0.2 to change the damage boost.
	...
	for (i = 0; i < count; i++) {
		...
		if (state.isBadState() == true){
			BadCheck=true
		}
	}

	if (SkillControl.checkAndPushCustomSkill(virtualActive.unitSelf, virtualPassive.unitSelf, entry, true, 'Crazy-Beat') !== null && BadCheck === true){
		return damage + boost;
	}
	else{
		return damage;		
